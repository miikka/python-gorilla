    ...
    for current_float in input[1:]:
        ...
        if xor_result.all(0):
            ...
            if current_leading >= prev_leading and current_trailing == prev_trailing:
                ...
                assert meaningful_count > 0, f"meaningful count must be non-zero, got {meaningful_count}"
                ...
                stream.insert(
                    xor_result[current_leading : (64 - current_trailing)]
                )
...
def decode(input: Bits) -> list[float]:
    ...
    while stream.pos < stream.length:
        if stream.read('bool') is False:
            ...
            xor_result = Bits(current_leading) + meaningful_bits + Bits(current_trailing)
            current_bits = prev_bits ^ xor_result
